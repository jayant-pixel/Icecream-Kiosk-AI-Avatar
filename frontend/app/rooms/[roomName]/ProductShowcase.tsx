...
// FIX: agent sends `locations` array, not `directions`. Using the wrong field
// name caused the directions panel to always render empty.
export type DirectionsPayload =
  | { action: "show"; display?: string; displayName?: string; locations?: DirectionEntry[] }
...
const formatPrice = ({
  ...
  if (typeof aed === "number") return `AED ${aed.toFixed(2)}`;
  if (typeof dollars === "number") return `AED ${dollars.toFixed(2)}`;
  if (typeof cents === "number") return `AED ${(cents / 100).toFixed(2)}`;
  return "";
};

const normalizeDisplayNames = (displayName?: string[] | string) => {
  if (!displayName) return [];
  return Array.isArray(displayName) ? displayName : [displayName];
};
...
export type ProductShowcaseProps = {
  className?: string;
  // directions prop removed — VideoConference no longer threads this through.
  // OverlayLayer now owns the directions panel exclusively.
};
...
export function ProductShowcase({ className }: ProductShowcaseProps = {}) {
  ...
  // NOTE: client.products RPC is registered here but the current agent never
  // calls it. These panels will only become active if the agent is updated to
  // emit `client.products` RPCs. OverlayLayer handles all current UI traffic.
  useEffect(() => {
    if (!room) return;
    ...
    const handleProductRpc = async (data: RpcInvocationData): Promise<string> => {
      try {
        ...
        switch (payload.action) {
          ...
          case "detail": {
            setMode("detail");
            setDetail(payload.products?.[0] ?? null);
            if (payload.query !== undefined) setQuery(payload.query ?? null);
            setSummary(null);
            setPendingProductId(null);
            setMenuCards((prev) => {
          ...
          case "added": {
            ...
            const cartShouldReset =
              ...
              (totalDollars !== null && totalDollars <= 0);
            if (cartShouldReset) setCartItems([]);
                ...
                if (productKey) {
                  ...
                  setCartItems((prev) => {
                    ...
                    if (index >= 0) {
                      ...
                      next.push({ key: productKey, qty: qtyToAdd, card: { ...payload.product } });
                    }
                    return next;
                  });
                }
    ...
    return () => room.unregisterRpcMethod("client.products");
  }, [room]);
  ...
  const handleAddToCartClick = async (card: ProductCard) => {
    ...
    setPendingProductId(String(productId));
    try {
  ...
  const normalizedCategory = useMemo(() => {
    ...
    return categories.some((c) => c.slug === activeCategory)
      ? activeCategory
  ...
  const activeCategoryLabel =
    normalizedCategory === "all"
      ? CATEGORY_FALLBACK
      : (categories.find((c) => c.slug === normalizedCategory)?.label ?? CATEGORY_FALLBACK);
  ...
  const resolveSummaryValue = (base: "subtotal" | "tax" | "total") => {
    ...
    return (
      formatPrice({
        aed: typeof summary[aedKey] === "number" ? (summary[aedKey] as number) : undefined,
        dollars:
          typeof summary[dollarsKey] === "number" ? (summary[dollarsKey] as number) : undefined,
        cents:
          typeof summary[centsKey] === "number" ? (summary[centsKey] as number) : undefined,
      }) || "--"
    );
  };

  const summaryMessage =
    typeof summary?.["message"] === "string" ? (summary["message"] as string) : null;

  const renderCategoryButtons = (orientation: "vertical" | "horizontal") => {
    if (!navCategories.length) return null;
            ...
            <div className="space-y-4 flex-1">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="text-xl font-bold text-[color:var(--icecream-primary)] bg-black/5 px-3 py-1 rounded-xl shadow-sm">
                  {formatPrice({ aed: primaryCard.priceAED })}
                </span>
              </div>
              ...
              {primaryLabels.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {primaryLabels.map((label) => (
                    <span
                      key={`${primaryCard.id ?? primaryCard.name}-${label}`}
                      className="rounded-full bg-black/5 border border-black/5 px-3 py-1 text-xs font-semibold text-black/60 shadow-sm"
                    >
                      {label}
                    </span>
                  ))}
              ...
              <div className="pt-4">
                ...
                {!agent ? (
                  <p className="mt-3 text-xs text-center font-medium text-black/40">
                    Scoop is getting ready...
                  </p>
                ) : null}
          ...
          <div className="relative flex-1 w-full overflow-hidden rounded-[32px] bg-white p-6 shadow-2xl border border-black/5">
            ...
            <div className="space-y-2 mb-6">
              ...
              <p className="text-sm font-medium text-black/60">
                Scoop is keeping it chilled while you finish up.
              </p>
            </div>
            {hasCartItems ? (
                  ...
                  {cartItems.map(({ key, card, qty }) => (
                    ...
                    >
                      <div className="relative h-20 w-full overflow-hidden rounded-xl bg-black/5">
                        {card.imageUrl ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={card.imageUrl} alt={card.name ?? "Treat"} className="h-full w-full object-contain p-1" />
                        ) : (
                      ...
                      <p className="line-clamp-2 text-xs font-bold text-[color:var(--icecream-dark)] leading-tight">
                        {card.name ?? "Treat"}
                      </p>
                      <p className="text-[11px] font-medium text-black/50">
                        {formatPrice({ aed: card.priceAED ?? undefined })}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className="mt-4 space-y-3">
              <p className="text-xs text-center font-medium text-black/50">
                Settle up at the counter whenever you&rsquo;re ready.
              </p>
              <button
                type="button"
      ...
      <div className="rounded-[40px] border border-black/5 bg-white p-8 shadow-2xl text-[color:var(--icecream-dark)]">
        <div className="flex flex-col gap-8 lg:flex-row">
          ...
          <div className="flex-1 max-w-full">
            ...
            <div className="mb-6 flex gap-2 overflow-x-auto pb-2 lg:hidden scrollbar-hide">
              {renderCategoryButtons("horizontal")}
            </div>
                      ...
                      <div className="mt-5 flex flex-1 flex-col px-1">
                        <div className="flex justify-between items-start gap-2">
                          ...
                          <span className="shrink-0 text-sm font-bold text-[color:var(--icecream-primary)] bg-[color:var(--icecream-primary)]/5 px-2 py-1 rounded-lg">
                            {formatPrice({ aed: item.priceAED })}
                          </span>
                        </div>
                        {item.description ? (
                          <p className="mt-2 text-sm font-medium leading-relaxed text-black/50 line-clamp-2">
                            {item.description}
                          </p>
                        ) : null}
                      </div>
          ...
          <div className="flex items-center gap-4 rounded-2xl bg-[color:var(--icecream-dark)] px-5 py-4 text-white shadow-2xl border border-white/10">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs opacity-70 font-medium mt-0.5">
                {formatPrice({ aed: toast.product.priceAED ?? undefined })}
              </p>
            </div>
          </div>
        </div>
